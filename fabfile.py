...
def install_spatial():
    ...
    with virtualenv():
        run('pip install --upgrade geojson geometryIO fiona rasterio utm')


@task
def install_node():
    'Install node.js server'
    install_library('http://nodejs.org/dist/v0.10.32/node-v0.10.32.tar.gz', 'node', yum_install='openssl-devel')
    with virtualenv():
